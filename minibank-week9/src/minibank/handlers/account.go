...
func getSessionLookup() func(string) (string, bool) {
	...
	if models.CassandraEnabled {
		return func(session string) (string, bool) {
			...
			if err := models.CassandraSession.Query("SELECT username FROM sessions WHERE session = ?", session).Scan(&username); err != nil {
			log.Fatal(err)
			}
...
func getSessionWriter() func(uuid.UUID, string) {
	...
	if models.CassandraEnabled {
		return func(session uuid.UUID, username string) {
			...
			if err := models.CassandraSession.Query("INSERT INTO sessions (session, username, expiration) VALUES (?, ?, ?)",
				session.String(),
				username,
...
func getSessionListLookup() func(string) UserSessions {
	...
	if models.CassandraEnabled {
		return func(username string) UserSessions {
			iter := models.CassandraSession.Query("SELECT session FROM sessions WHERE username =?", username).Iter()
			sessionList := []string{}
			var session string
